...
function buildListingWhere(req: AuthRequest): Prisma.ListingWhereInput {
  ...
  const where = { status: "PUBLISHED" } as unknown as Prisma.ListingWhereInput
...
function attachListingCover<L extends { photos?: { url: string }[]; url: string | null }>(listing: L): L & { coverUrl: string | null } {
  const coverUrl = listing.photos?.[0]?.url ?? listing.url ?? null;
  return { ...listing, coverUrl };
}

function attachListingCovers<L extends { photos?: { url: string }[]; url: string | null }>(listings: L[]) {
  return listings.map(attachListingCover);
}

function parsePhotoInputs(body: Record<string, unknown>) {
  const urls: string[] = [];
...
export async function getAllListings(req: AuthRequest, res: Response) {
  try {
    ...
    let isBookedByMeMap: Record<string, true> = {}
...
export async function getMineListings(req: AuthRequest, res: Response) {
  try {
    if (!req.userId) {
...
export async function getPendingListings(_req: AuthRequest, res: Response) {
  try {
    const listings = await prisma.listing.findMany({
    ...
    const guestCount =
      guests !== undefined && guests !== null
        ...
        : guest !== undefined && guest !== null
          ? Number(guest)
          : 1;
    const photoUrls = parsePhotoInputs(req.body as Record<string, unknown>);
...
export async function updateListingStatus(req: AuthRequest, res: Response) {
  try {
    const id = req.params["id"] as string;
