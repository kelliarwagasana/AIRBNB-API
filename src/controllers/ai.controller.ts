...
const sessionHistory = new Map<string, ChatHistoryItem[]>();
  ...
  return {
    location:
      typeof raw["location"] === "string" && raw["location"].trim().length > 0
        ? raw["location"].trim()
        : null,
    type: rawType && validTypes.includes(rawType as ListingType) ? (rawType as ListingType) : null,
    maxPrice:
      typeof raw["maxPrice"] === "number" && Number.isFinite(raw["maxPrice"]) && raw["maxPrice"] > 0
        ? raw["maxPrice"]
        : null,
    guests:
      typeof raw["guests"] === "number" && Number.isFinite(raw["guests"]) && raw["guests"] > 0
        ? Math.floor(raw["guests"])
        : null,
  };
}

function hasAtLeastOneFilter(filters: SearchFilters) {
...
const structuredExtractor = aiConfig.aiSearchModel.withStructuredOutput(
  aiConfig.listingSearchFilterSchema,
  {
    name: "extract_listing_search_filters",
  }
);

export async function searchListingsWithAI(req: Request, res: Response) {
  try {
    ...
    const extracted = await structuredExtractor.invoke([
      new SystemMessage(
        "Extract listing search filters from the user's request. Return only supported filters. If a filter cannot be inferred, return null for that field."
      ),
      new HumanMessage(query),
    ]);

    const filters = normalizeFilters(extracted as Record<string, unknown>);
    ...
    if ((error as Record<string, any>)?.status === 429) {
      return res.status(429).json({ error: "AI service is busy, please try again in a moment" });
    }
...
function buildListingDescriptionPrompt(listing: { title: string; location: string; pricePerNight: number; guests: number; type: ListingType; amenities: string[]; description: string }, tone: string) {
...
function buildChatSystemPrompt(listing?: { title: string; location: string; pricePerNight: number; guests: number; type: ListingType; amenities: string[]; description: string }) {
  if (!listing) {
...
function buildBookingHistorySummary(bookings: Array<{ listing: { location: string; type: ListingType; pricePerNight: number; guests: number; title: string } }>) {
...
Based on this booking history, infer the user's preferences and return a JSON object with preferences, searchFilters, and reason. Only return valid JSON in this format:
{
  ...
  "reason": "string explaining the recommendation"
    ...
    const parsed = parseJson<{ preferences: string; searchFilters: { location: string | null; type: string | null; maxPrice: number | null; guests: number | null }; reason: string }>(String(aiResponse));

    const filterClauses: Prisma.ListingWhereInput[] = [
      {
        id: {
    ...
    if (parsed.searchFilters.type) {
      filterClauses.push({ type: parsed.searchFilters.type as ListingType });
    }
...
export async function getListingReviewSummary(req: Request, res: Response) {
  try {
    const listingId = req.params["id"] as string;
    ...
    const formattedReviews = reviews
      .map(
        (review, index) =>
          `Review ${index + 1} (rating: ${review.rating}) by ${review.reviewer.name}: ${review.comment ?? "No comment"}`
      )
      .join("\n");
...
${formattedReviews}`;
    ...
    const parsed = parseJson<{ summary: string; positives: string[]; negatives: string[] }>(String(aiResponse));
